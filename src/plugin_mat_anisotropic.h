...
/**
 * @file
 * @brief Defines material plugin class CyclesAnisotropicMat.
 */
...
/**
 * @brief Material class that allows the use of the Cycles Anisotropic shader.
 */
class CyclesAnisotropicMat : public CyclesPluginMatBase {
public:
	CyclesAnisotropicMat();

	// Paramblock access
	AnisotropicDistribution GetDistribution(TimeValue t, Interval& iv);
	...
	float GetAnisotropy(TimeValue t, Interval &iv);
	Texmap* GetAnisotropyTexmap();
	float GetRotation(TimeValue t, Interval &iv);
	Texmap* GetRotationTexmap();
