...
// Pblock enum
enum { glass_pblock, normal_pblock };
...
static ParamBlockDesc2 mat_glass_pblock_desc(
	// Pblock data
	glass_pblock,
	_T("pblock"),
	0,
	GetCyclesMaterialGlassClassDesc(),
	...
	NULL,
		...
		P_ANIMATABLE,
		IDS_DISTRIBUTION,
		p_default, GlassDistribution::GGX,
		p_range, GlassDistribution::MULTISCATTER_GGX, GlassDistribution::BECKMANN,
		p_ui, TYPE_RADIO, 3, IDC_RADIO_DIST_MULTI_GGX, IDC_RADIO_DIST_GGX, IDC_RADIO_DIST_BECKMANN,
		p_end,
		// Color
		param_color,
		_T("color"),
...
MAKE_COMMON_NORMAL_PBLOCK(normal_pblock, GetCyclesMaterialGlassClassDesc())

CyclesGlassMat::CyclesGlassMat() : CyclesPluginMatBase(true, true)
{
	pblock = nullptr;
...
GlassDistribution CyclesGlassMat::GetDistribution(const TimeValue t, Interval& iv)
{
	const int distribution = pblock->GetInt(param_dist, t, iv);

	if (distribution < 0 || distribution >= static_cast<int>(GlassDistribution::COUNT)) {
		return GlassDistribution::GGX;
	}

	return static_cast<GlassDistribution>(distribution);
}

Color CyclesGlassMat::GetGlassColor(const TimeValue t, Interval& iv)
{
	return pblock->GetColor(param_color, t, iv);
}

Texmap* CyclesGlassMat::GetGlassColorTexmap()
{
...
float CyclesGlassMat::GetRoughness(const TimeValue t, Interval iv)
{
	return pblock->GetFloat(param_roughness, t, iv);
}

Texmap* CyclesGlassMat::GetRoughnessTexmap()
{
...
float CyclesGlassMat::GetIOR(const TimeValue t, Interval iv)
{
	return pblock->GetFloat(param_ior, t, iv);
}

ClassDesc2* CyclesGlassMat::GetClassDesc() const
{
	return GetCyclesMaterialGlassClassDesc();
}

ParamBlockDesc2* CyclesGlassMat::GetParamBlockDesc()
{
	return &mat_glass_pblock_desc;
}

ParamBlockDesc2* CyclesGlassMat::GetNormalParamBlockDesc()
{
	return &common_normal_pblock_desc;
}

void CyclesGlassMat::PopulateSubmatSlots()
{
	// No sub-materials, do nothing
}

void CyclesGlassMat::PopulateSubtexSlots()
{
...
void CyclesGlassMat::Update(const TimeValue t, Interval& valid)
{
	if (pblock == nullptr) {
		return;
	}
