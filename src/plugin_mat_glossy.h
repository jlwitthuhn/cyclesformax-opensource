...
/**
 * @file
 * @brief Defines material plugin class CyclesGlossyMat.
 */

#include "plugin_mat_base.h"

 /**
  * @brief Material class that allows the use of the Cycles Glossy shader.
  */
class CyclesGlossyMat : public CyclesPluginMatBase {
public:
	CyclesGlossyMat();

	// Paramblock access
	GlossyDistribution GetDistribution(TimeValue t, Interval& iv);
	Color GetGlossyColor(TimeValue t, Interval& iv);
	Texmap* GetGlossyColorTexmap();
