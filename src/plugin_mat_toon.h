...
/**
 * @file
 * @brief Defines material plugin class CyclesToonMat.
 */

#include "plugin_mat_base.h"

 /**
  * @brief Material class that allows the use of the Cycles Toon shader.
  */
class CyclesToonMat : public CyclesPluginMatBase {
public:
	CyclesToonMat();

	// Paramblock access
	ToonComponent GetComponent(TimeValue t, Interval& iv);
	Color GetMatColor(TimeValue t, Interval& iv);
	Texmap* GetMatColorTexmap();
	float GetSize(TimeValue t, Interval& iv);
	Texmap* GetSizeTexmap();
	float GetSmooth(TimeValue t, Interval& iv);
	Texmap* GetSmoothTexmap();
