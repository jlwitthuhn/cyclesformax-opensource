...
static ParamBlockDesc2 mat_add_pblock_desc(
	...
	_T("pblock"),
	0,
	GetCyclesMaterialVolumeAddClassDesc(),
	P_AUTO_CONSTRUCT + P_AUTO_UI,
	PBLOCK_REF,
	// Interface stuff
	IDD_PANEL_MAT_VOL_ADD,
	IDS_PARAMS,
	0,
	0,
	NULL,
...
CyclesVolAddMat::CyclesVolAddMat() : CyclesPluginMatBase(false, false)
{
	pblock = nullptr;
...
Mtl* CyclesVolAddMat::GetMtlA()
{
	return GetSubMtl(submat_a);
}

Mtl* CyclesVolAddMat::GetMtlB()
{
	return GetSubMtl(submat_b);
}

ClassDesc2* CyclesVolAddMat::GetClassDesc() const
{
	return GetCyclesMaterialVolumeAddClassDesc();
}

ParamBlockDesc2* CyclesVolAddMat::GetParamBlockDesc()
{
	return &mat_add_pblock_desc;
}

void CyclesVolAddMat::PopulateSubmatSlots()
{
...
void CyclesVolAddMat::PopulateSubtexSlots()
{

}

void CyclesVolAddMat::Update(const TimeValue t, Interval& valid)
{
	if (pblock == nullptr) {
		return;
	}
