...
/**
 * @file
 * @brief Defines the translator class CyclesLightNodeTranslator.
 */
...
class CyclesLightNodeTranslator : public MaxSDK::RenderingAPI::BaseTranslator_ReferenceTarget
{
public:
	class TranslatorKey;

	CyclesLightNodeTranslator(const TranslatorKey& key, MaxSDK::RenderingAPI::TranslatorGraphNode& translator_graph_node);
	~CyclesLightNodeTranslator();

	bool GetLightParams(CyclesLightParams& light_params) const;
...
class CyclesLightNodeTranslator::TranslatorKey : public MaxSDK::RenderingAPI::TranslatorKey
{
public:
	TranslatorKey(INode* inode);

	INode* get_node() const;
...
private:
	static size_t compute_hash(INode* node);
