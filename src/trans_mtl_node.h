...
/**
 * @file
 * @brief Defines the translator class CyclesMtlNodeTranslator.
 */
...
public:
	...
	bool operator==(const NodeMaterialProperties& other) const;
	bool operator!=(const NodeMaterialProperties& other) const;
};

class CyclesMtlNodeTranslator : public MaxSDK::RenderingAPI::BaseTranslator_ReferenceTarget
{
public:
	class TranslatorKey;

	CyclesMtlNodeTranslator(const TranslatorKey& key, MaxSDK::RenderingAPI::TranslatorGraphNode& translator_graph_node);
	~CyclesMtlNodeTranslator();

	bool GetNodeMaterialProperties(NodeMaterialProperties& mtl_properties) const;
...
class CyclesMtlNodeTranslator::TranslatorKey : public MaxSDK::RenderingAPI::TranslatorKey
{
public:
	TranslatorKey(Mtl* mtl);
...
private:
	static size_t compute_hash(Mtl* mtl);
